...
const handle_request = async (request) => {
	...
	if (res_data == null) {
		return { status: false, code: 201, message: `Unable to update department` }
	}	
	log.info(`[${request.auth.credentials.company_oid}/${request.auth.credentials.login_id}] - department update - ${request.payload.name}`)
