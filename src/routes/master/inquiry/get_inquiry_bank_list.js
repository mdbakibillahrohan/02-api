...
const get_list = {
    method: "GET",
    path: API.CONTEXT + API.MASTER_INQUIRY_GET_BANK_LIST,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
        },
        description: "Inquiry bank list",
...
const handle_request = async (request) => {
    try {
        ...
        log.info(`inquiry bank info found`);
        return {
            status: true,
            code: 200,
...
const get_data = async (request) => {
    let list_data = [];
    let data = [];
    let query = `select oid, nameEn, nameBn from ${TABLE.BANK} where 1 = 1`;
