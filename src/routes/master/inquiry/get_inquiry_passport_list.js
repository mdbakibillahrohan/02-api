...
const get_list = {
    method: "GET",
    path: API.CONTEXT + API.MASTER_INQUIRY_GET_PASSPORT_LIST,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
        },
        description: "Inquiry passport list",
...
const handle_request = async (request) => {
    try {
        ...
        log.info(`inquiry passport info found`);
        return {
            status: true,
            code: 200,
        ...
        log.error(`An exception occurred while getting inquiry passport list data : ${err?.message}`);
        return {
            status: false,
            code: 500,
