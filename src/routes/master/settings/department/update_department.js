...
const save_controller = {
    method: "POST",
    path: API.CONTEXT + API.MASTER_SETTINGS_UPDATE_DEPARTMENT,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
        },
        description: "update setting department information",
...
const handle_request = async (request) => {
    ...
    try {
        ...
        log.error(`An exception occurred while updating setting department info: ${err}`);
...
const update_data = async (request) => {
    try {
        ...
        const update_department = await updateDepartment(request);
        if (!update_department) {
            return {
                status: false,
                message: "Problem in update department"
            }
        }
        return {
            status: true
...
const updateDepartment = async (request) => {
    ...
    let scols = cols.join(', ')
    let query = `update ${TABLE.DEPARTMENT} set ${scols} where 1 = 1 and oid = $${idx++}`;
    let sql = {
        text: query,
        values: data
    }
