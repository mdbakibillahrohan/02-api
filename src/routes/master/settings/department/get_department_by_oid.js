...
const get_by_oid = {
    method: "GET",
    path: API.CONTEXT + API.MASTER_SETTINGS_GET_DEPARTMENT_BY_OID,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
        },
        description: "get setting department by oid",
...
const handle_request = async (request) => {
    try {
        ...
        log.info(`department data found by oid`);
        return {
            status: true,
            code: 200,
...
const get_data = async (request) => {
    ...
    query = `select oid, nameEn, nameBn, status, sortOrder
    from ${TABLE.DEPARTMENT} where 1 = 1 and companyOid = $1 and oid = $2`;
    let sql = {
        text: query,
