...
const get_by_oid = {
    method: "GET",
    path: API.CONTEXT + API.MASTER_SETTINGS_GET_DESIGNATION_BY_OID,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
        },
        description: "get setting designation by oid",
...
const handle_request = async (request) => {
    try {
        ...
        log.info(`designation data found by oid`);
        return {
            status: true,
            code: 200,
...
const get_data = async (request) => {
    ...
    query = `select d.oid, d.nameEn, d.nameBn, d.status, d.sortOrder
    from ${TABLE.DESIGNATION} d
