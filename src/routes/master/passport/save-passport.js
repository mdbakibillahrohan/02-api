...
const handle_request = async (request) => {
	try {
        ...
        if (res_data.status) {
            	log.info(`[${request.auth.credentials.company_oid}/${request.auth.credentials.login_id}] - passport save - ${request.payload.name}`)
            return { status: true, code: 200, message: `Successfully save ${request.payload.full_name}` }
        }
...
const save_passport = async (request, user_info, passport_oid) => {
    let executed
    let index = 1
    let cols = ['oid', 'full_name', 'sur_name', 'given_name', 'gender', 'nationality', 'passport_number', 'people_oid', 'company_oid', 'created_on', 'created_by']
    let params = [`$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`,`$${index++}`]

    let data = [passport_oid, request.payload.full_name, request.payload.sur_name, request.payload.given_name, request.payload.gender, request.payload.nationality, request.payload.passport_number, request.payload.people_oid, user_info.company_oid, 'now()', user_info.login_id]
...
const save_passport_Detail = async (request, user_info, passport_detail, sort_order_no, passport_oid) => {
    let executed
    let index = 1
    let cols = ['oid', 'title', 'image_path', 'sort_order', 'passport_oid', 'company_oid']
    let params = [`$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`, `$${index++}`]
    const oid = uuid.v4()
    let data = [oid, passport_detail.title, passport_detail.image_path, sort_order_no, passport_oid, user_info.company_oid]
...
const get_count = async (request) => {
    let count = 0
    let data = [request.payload.passport_number, request.payload.people_oid, request.auth.credentials.company_oid]

    let query = `select count(*) as total from ${TABLE.PASSPORT}  where 1 = 1 and passport_number = $1 and people_oid = $2 and company_oid = $3`
