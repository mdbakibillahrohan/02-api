...
const { API, TABLE, SCHEMA, CONSTANT } = require("../../../util/constant")

const route_controller = {
    method: "POST",
    path: API.CONTEXT + API.AUTHENTICATION_USER_LOGOUT_PATH,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
...
const update_data = async (request, token) => {
    let sql = {
        text: `update ${TABLE.LOGIN_TRAIL} set logoutTime = clock_timestamp(), status = $1 
            where 1 = 1 and access_token = $2`,
        values: [CONSTANT.LOGOUT, token]
    }
    try {
