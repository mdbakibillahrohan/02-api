...
const payload_scheme = Joi.object({
	...
	status: Joi.array().items(Joi.string().trim().optional().allow(null, '').valid('Active', 'Inactive')).optional(),
	people_type: Joi.array().items(Joi.string().trim().optional().allow(null, '').valid('Employee', 'Customer', 'Supplier')).optional(),
})
const route_controller = {
	method: "POST",
	path: API.CONTEXT + API.GET_PEOPLE_LIST,
	options: {
		auth: {
			mode: "required",
			strategy: "jwt",
...
const get_count = async (request) => {
	...
	if (request.payload.people_type && request.payload.people_type.length > 0) {
		let people_type = request.payload.people_type.map((x) => `p.people_type::text ilike '%${x}%'`).join(' or ')
...
const get_data = async (request) => {
	let index = 1
	let data, param = []
	let query = `select p.oid, p.name, p.email, p.company_oid, p.status, p.image_path, p.mobile_no,
		p.payable_balance::float8, p.receivable_balance::float8, c.name as company_name, d.name as designation,
		(
