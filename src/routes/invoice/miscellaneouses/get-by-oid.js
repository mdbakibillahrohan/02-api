...
const handle_request = async (request) => {
    try {
        ...
        try{
            ...
            log.error(`An exception occurred  : ${err?.message}`)
            return {
                status: false,
                code: 500,
        ...
        log.error(`An exception occurred while getting Miscellaneouses Invoice information by oid : ${err?.message}`);
    }
}
...
const getMiscellaneousesDetailByOid = async (userInfo, request) => {
    ...
    try {
        ...
        log.error( `An exception occurred while getting Miscellaneouses Detail by ticketInvoiceOid : ${ err?.message }`)
        // throw new Error(e);
    }
    return list_data;
}
