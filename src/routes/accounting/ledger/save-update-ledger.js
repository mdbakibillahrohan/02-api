...
const route_controller = {
	method: "POST",
	path: API.CONTEXT + API.ACCOUNTING_LEDGER_SAVE_UPDATE_PATH,
	options: {
		auth: {
			mode: "required",
			strategy: "jwt",
		},
		description: "save/update ledger",
...
const handle_request = async (request) => {
	...
	if (res_data == null) {
		return { status: false, code: 201, message: `Unable to save/update ledger` }
	}
	log.info(`[${request.auth.credentials.company_oid}/${request.auth.credentials.login_id}] - ledger save/update - ${request.payload.ledger_name}`)
...
const post_data = async (request) => {
	...
	try {
		...
		log.error(`An exception occurred while saving/updating ledger : ${e?.message}`)
	}
	return data
}
