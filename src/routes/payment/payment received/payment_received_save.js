...
const save_data = async (request) => {
    try {
        ...
        const due_invoice_list_execute = due_invoice_list.forEach(async (element) => {
            if (element.amount>0) {
                const execute = await saveInvoiceBillPayment(request, element, request.payload.oid);
                if (!execute) {
                    return false;
                }
...
const savePaymentReceived = async (request) => {
    const { oid,payment_no, payment_date, amount, image_path, status, payment_type, reference_type, reference_oid, payment_nature, account_oid, userInfo, payment_mode, check_no, description } = request.payload;
    let executed;
    let idx = 1;
