...
const query_scheme = Joi.object({
    ...
    reference_oid: Joi.string().trim().min(1).optional(),
    payment_type: Joi.string().trim().min(1).optional(),
    reference_type: Joi.string().trim().min().optional(),
    payment_nature: Joi.string().trim().min(1).optional()
})

const route_controller = {
    method: "GET",
    path: API.CONTEXT + API.PAYMENT_RECEIVED_GET_DUE_INVOICE_LIST,
    options: {
        auth: {
            mode: "required",
            strategy: "jwt",
        },
        description: "Get due invoice list",
        plugins: {
